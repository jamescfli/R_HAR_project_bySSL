...
# # load 6 axis data from accelerometer and gyroscope, xyz-axis
...
# # driving
# # it is ok to use either .driving1 or .driving2
.connectToDb <- dbConnect(drv=RSQLite::SQLite(), dbname="DataSets/Training/JAMES_measure_DRIVING_151217T205325.db")
.driving1 <- dbGetQuery(conn=.connectToDb, statement="SELECT * FROM measurements")
drivingData <- as.matrix(.driving1)[81:12080, c(-1,-2,-6)]
colnames(drivingData) <- c("aX","aY","aZ","gX","gY","gZ")
# # 12263 samples in .driving1
